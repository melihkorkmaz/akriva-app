...
import {
  ...
  updateEmissionSourceSchema,
} from "$lib/schemas/emission-source.js";
import type {
...
export const actions: Actions = {
    ...
    try {
      ...
      if (err instanceof ApiError) {
        if (err.status === 400 && err.body.code === "VALIDATION_FAILED") {
          return message(form, err.body.error || "Validation failed.", {
            status: 400,
          });
        }
        if (err.status === 404) {
    ...
    try {
      ...
      if (form.data.overrideScientificAuthority) {
        ...
        if (rawGwp !== "ar5" && rawGwp !== "ar6") {
          return message(form, "GWP version is missing or invalid.", {
            status: 400,
          });
        }
        gwpVersion = rawGwp;
    ...
    try {
      ...
      if (err instanceof ApiError) {
        ...
        if (err.status === 409) {
          return message(
            form,
            "An emission source with this name already exists.",
            { status: 409 }
          );
        }
        if (err.status === 403) {
        ...
        if (err.status === 422) {
          return message(
            form,
            err.body.error || "Invalid unit for this category.",
            { status: 422 }
          );
        }
      }
  ...
  updateSource: async ({ request, locals }) => {
    ...
    const form = await superValidate(
      formData,
      zod4(updateEmissionSourceSchema)
    );
