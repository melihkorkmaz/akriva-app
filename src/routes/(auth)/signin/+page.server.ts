...
export const actions: Actions = {
    ...
    try {
      ...
      if (response.type === "mfa_challenge") {
        return {
          ...
          mfaSession: response.challenge.session,
        };
      }
  ...
  mfa: async ({ request, cookies }) => {
    const form = await superValidate(request, zod4(mfaVerifySchema), {
      id: "mfa",
    });
    ...
    try {
      ...
      if (err instanceof ApiError && err.status === 401) {
        return message(form, "Invalid verification code. Please try again.", {
          status: 401,
        });
      }
